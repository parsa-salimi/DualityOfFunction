    ...
    (cond [(or (empty? f) (empty? g)) (list #t (list f g))]
          ...
          [ else
            (letrec ((x (tiebreaker (pivot (vars f) f g)))
                 ...
                 (list (list f g) ;node
                       (FK-treelist (reduce f1) (reduce  (disjunction g0 g1)) pivot tiebreaker (remove x varlist)) ;left subtree
                       (FK-treelist (reduce (disjunction f0 f1)) (reduce g1) pivot tiebreaker (remove x varlist))))])) ;right subtree
